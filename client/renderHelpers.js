...
export const renderAllPlayers = (playerList) => {
  ...
  for (let i = 0; i < detailButtons.length; i++) {
    ...
    const response = await fetchSinglePlayer(button.dataset.id);
    renderSinglePlayer(response);
    });
  }
};
...
export const renderSinglePlayer = (playerObj) => {
  ...
  const button = document.querySelector('#see-all');
  button.addEventListener('click', async (event) => {
  const players = await fetchAllPlayers(button);
